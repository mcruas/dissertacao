...
\citeonline{vieu_nonparametric_2006} formaliza a diferença entre métodos paramétricos e métodos não-paramétricos.
...
Uma referência para aprofundar mais no estudo de splines pode ser encontrada em \citeonline{de_boor_practical_1978}. Um exemplo de base de B-splines é mostrado na figura~\ref{fig:base-bspline}.
\begin{figure}[h!] 
  \centering
...
\citeonline{ferraty_2007} investiga aspectos teóricos sobre a regressão com NP-FDA, mostrando uma aplicação do método aplicado a um problema de quimiometria. Já \citeonline{barrientos-marin_locally_2010} implementa a generalização do estimador \emph{local linear} para o caso funcional.
...
\todo[inline]{Escrever seção: semelhante à NPFDA}
...
\todo[inline]{Dúvida: DM em ambos ou utilizar GW na janela movel? Significância bilateral?}
...
A primeira pergunta a ser respondida, quando a série começou a ser analisada, é com relação à sua estacionariedade. Para ser considerada estacionária, na sua forma fraca, a série deve ser gerada por um processo de média e desvio padrão constantes. Em muitos casos, lança-se mão da diferenciação sucessiva da série para transformar os dados brutos numa série estacionária. Neste trabalho, conforme indicado na seção~\ref{metodologia-npfda}, utiliza-se a subtração de séries distintas para tentar obter uma série temporal mais bem comportada. Para todos os valores de $k_t$ descritos, com exceção de quando é identicamente nulo, a série não teve sua estacionariedade rejeitada pelo teste ADF, para nenhuma maturidade. Decidiu-se realizar o mesmo processo de retirar valores das curvas na estimação das séries temporais funcionais. Os valores de $k_t$ utilizados são os mesmos definidos na seção~\ref{metodologia-npfda}.
...
\begin{figure}[htp]
  ...
  \begin{minipage}[t]{0.45\linewidth}
    ...
    \begin{minipage}[t]{\linewidth}
     ...
     \subcaption{Horizonte 1}
    \end{minipage}
    \begin{minipage}[b]{\linewidth}
      \centering     \includegraphics[width=\textwidth]{anexos/heatmap3.pdf} \subcaption{Horizonte 3}
    \end{minipage}
  \end{minipage}
  \begin{minipage}[t]{0.45\linewidth}
    \centering
	...
	\subcaption{Horizonte 6}
    \end{minipage}
    \begin{minipage}[b]{\linewidth}
      \centering     \includegraphics[width=\textwidth]{anexos/heatmap12.pdf} \subcaption{Horizonte 12}
    \end{minipage}
  \end{minipage}
...
\todo[inline]{Colocar os gráficos de calor: os 4 horizontes (janela movel ou expandida?). Colocar o gráfico com os erros acumulados, 4 horizontes, melhor de ftsa, npf, DL e o ar1.}
...
\todo[inline]{explicar nome dos métodos na tabela}
